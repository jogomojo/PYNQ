...
class RFdc(DefaultIP):
    ...
    def __init__(self, description, clock_config=None, enabled_tiles=None):
        ...
        # Extract size
        RFDC_DEFAULT_MEM_SIZE = 0x40000  # 256KB — typical RFDC register region
        size = RFDC_DEFAULT_MEM_SIZE
        if 'addr_range' in description:
            if isinstance(description['addr_range'], list) and len(description['addr_range']) >= 2:
                size = description['addr_range'][1] - description['addr_range'][0]
            elif isinstance(description['addr_range'], int):
                size = description['addr_range']
    ...
    def _detect_enabled_tiles(self):
        ...
        enabled = {'dac': [], 'adc': []}
        for i in range(4):
            if i < len(self.config_pb.dac_tile_config):
                if self.config_pb.dac_tile_config[i].enable:
                    enabled['dac'].append(i)
            if i < len(self.config_pb.adc_tile_config):
                if self.config_pb.adc_tile_config[i].enable:
